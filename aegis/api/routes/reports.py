...
async def download_report(
    ...
    if format_type == "markdown":
        ...
        filename = f"aegis_report_{report.campaign.name}_{report.id[:8]}.md"
    elif format_type == "json":
        media_type = "application/json"
        filename = f"aegis_report_{report.campaign.name}_{report.id[:8]}.json"
    else:
        media_type = "text/html"
        filename = f"aegis_report_{report.campaign.name}_{report.id[:8]}.html"
